...
When using per collection statistics, extra permissions for the listCollection
action are needed per db.  You can provide these extra permissions e.g. via:
```
> db.runCommand({createRole:"listCollectionsMonitor", privileges: [{resource: {db: "", collection: ""}, actions:["listCollections"]}], roles: []})
> db.grantRolesToUser("user", ["listCollectionsMonitor"])
```
...
mongo_coll_stats,coll_name=startup_log,db_name=local,hostname=localhost:27017 ok=1i,type="coll_stat",storage_size=36864i,last_extent_size=0i,capped=true,size=5304i,max_size=10485760i,avg_obj_size=1768,num_extents=0i,count=3i,nindexes=1i,total_index_size=36864i,max=-1i 1524653759000000000
```
